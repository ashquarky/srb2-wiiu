...
static GX2VertexShader* (*GLSL_CompileVertexShader)(const char* shaderSource, char* infoLogOut, int infoLogMaxLength, int32_t flags);
static GX2PixelShader* (*GLSL_CompilePixelShader)(const char* shaderSource, char* infoLogOut, int infoLogMaxLength, int32_t flags);
...
static bool GLSL_Shutdown(void)
{
#ifdef GLSL_COMPILER_CAFE_RPL
    if (s_glslCompilerModule == NULL)
        return false;
