// Ash: modified for C compatibility 2024-09-09
...
static OSDynLoad_Module s_glslCompilerModule = NULL;
#endif
...
static GX2VertexShader* (*GLSL_CompileVertexShader)(const char* shaderSource, char* infoLogOut, int infoLogMaxLength, enum GLSL_COMPILER_FLAG flags);
static GX2PixelShader* (*GLSL_CompilePixelShader)(const char* shaderSource, char* infoLogOut, int infoLogMaxLength, enum GLSL_COMPILER_FLAG flags);
static void (*GLSL_FreeVertexShader)(GX2VertexShader* shader);
static void (*GLSL_FreePixelShader)(GX2PixelShader* shader);
static void (*__GLSL_DestroyGLSLCompiler)();
...
{
	void (*_InitGLSLCompiler)() = NULL;
#if defined(__WUT__) || defined(__WIIU__)
	if (s_glslCompilerModule != NULL)
		return false;
